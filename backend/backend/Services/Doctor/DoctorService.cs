...
namespace backend.Services
{
    public class DoctorService : IDoctorService
    {
        ...
        public async Task<List<object>> FilterDoctorsWithDetailsAsync(string? departmentId, string? specialtyId, string? branchId)
        {
            var doctors = await _doctorCollection.Find(_ => true).ToListAsync();
            var details = await _doctorDetailCollection.Find(_ => true).ToListAsync();

            var result = (from d in doctors
                          join dt in details on d.IdDoctor equals dt.DoctorId into gj
                          from subdt in gj.DefaultIfEmpty()
                          where
                              (string.IsNullOrEmpty(departmentId) || departmentId == "all" || subdt?.DepartmentId == departmentId) &&
                              (string.IsNullOrEmpty(specialtyId) || specialtyId == "all" || subdt?.SpecialtyId == specialtyId) &&
                              (string.IsNullOrEmpty(branchId) || branchId == "all" || subdt?.BranchId == branchId)
                          select new
                          {
                              Id = d.IdDoctor,
                              Name = d.Name,
                              ...
                              Img = subdt?.Img,
                              BranchName = subdt?.BranchName,
                              DepartmentName = subdt?.DepartmentName,
                              SpecialtyName = subdt?.SpecialtyName
                          }).ToList();
