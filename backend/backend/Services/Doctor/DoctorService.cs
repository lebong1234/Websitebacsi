...
namespace backend.Services
{
    public class DoctorService : IDoctorService
    {
        ...
        public DoctorService(IOptions<MongoDbSettings> mongoDbSettings, IWebHostEnvironment env, IMongoClient mongoClient)
        {
            ...
            _users = mongoDatabase.GetCollection<backend.Models.Entities.User>("Users");
        ...
        public async Task<(bool Success, string? ErrorMessage, Doctor? CreatedDoctor)> CreateDoctorWithDetailsAsync(CreateFullDoctorDto dto)
        {
            ...
            try
            {
                ...
                // Insert schedules for each date
                if (dto.TimeSlots != null && !string.IsNullOrEmpty(dto.Date))
                {
                ...
                // If you want to support multiple days at once, you can extend the DTO and logic here
        ...
        public async Task<(bool Success, string? ErrorMessage)> UpdateDoctorWithDetailsAsync(string doctorId, UpdateFullDoctorDto dto)
        {
            ...
            try
            {
                ...
                // 3. Cập nhật hoặc tạo DoctorSchedule (per-day, per-time-slot)
                if (dto.TimeSlots != null && !string.IsNullOrEmpty(dto.Date))
                {
                    var scheduleFilter = Builders<DoctorSchedule>.Filter.Where(ds => ds.DoctorId == doctorId && ds.Date == dto.Date);
                    var existingSchedule = await _doctorScheduleCollection.Find(session, scheduleFilter).FirstOrDefaultAsync();
                    if (existingSchedule == null)
                    {
                        ...
                        await _doctorScheduleCollection.InsertOneAsync(session, newSchedule);
                    }
                    else
                    {
                        existingSchedule.TimeSlots = dto.TimeSlots;
                        await _doctorScheduleCollection.ReplaceOneAsync(session, scheduleFilter, existingSchedule);
                    }
                }
