...
builder.Services.AddScoped<Net.payOS.PayOS>(provider =>
{
    ...
    var clientId = config["PayOS:ClientId"] ?? throw new Exception("Missing PayOS ClientId");
    var apiKey = config["PayOS:ApiKey"] ?? throw new Exception("Missing PayOS ApiKey");
    var checksumKey = config["PayOS:ChecksumKey"] ?? throw new Exception("Missing PayOS ChecksumKey");
