        ...
        public async Task<IActionResult> Index(string searchString)
        {
            ...
            if (!string.IsNullOrEmpty(searchString))
            {
                patients = patients.FindAll(p =>
                    (p.FullName?.Contains(searchString, StringComparison.OrdinalIgnoreCase) ?? false) ||
                    (p.PatientCode?.Contains(searchString, StringComparison.OrdinalIgnoreCase) ?? false) ||
                    (p.Phone?.Contains(searchString, StringComparison.OrdinalIgnoreCase) ?? false));
            }
