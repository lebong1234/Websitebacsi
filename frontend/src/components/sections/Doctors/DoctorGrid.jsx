  ...
  return (
    <div className="flex flex-wrap justify-center -mr-4">
      {doctors.map((doctor) => {
        ...
        if (!docId || typeof docId !== 'string' || docId.length !== 24) return null;
        return (
          <div key={docId} className="mr-6 mb-4" style={{ width: 'calc(25% - 45px)'  }}>
            <DoctorCard
              Id={docId}
              Name={doctor.Name || doctor.name}
              DoctorImage={doctor.DoctorImage || doctor.doctorImage || doctor.Img || doctor.img}
              SpecializingIn={doctor.SpecializingIn || doctor.specializingIn || doctor.SpecialtyName || doctor.specialtyName}
              WorkingAtBranch={doctor.WorkingAtBranch || doctor.workingAtBranch || doctor.BranchName || doctor.branchName}
              onClick={() => {
                console.log('DoctorGrid click:', doctor, 'docId:', docId);
                if (onDoctorClick) onDoctorClick(doctor);
              }}
            />
          </div>
        );
