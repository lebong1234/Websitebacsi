            ...
            {loading && !departmentsData.length ? renderLoading() : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {departmentsData.map(dept => (
                  ...
                  >
                    <img
                      src={dept.imageUrlDepartment ? `${API_IMG_URL}${dept.imageUrlDepartment}` : DEFAULT_PLACEHOLDER_IMAGE}
                      alt={dept.nameDepartment || "Khoa"}
                      className="w-16 h-16 object-cover mb-4 rounded-lg"
                      onError={handleImageError}
                    />
                    <h3 className="text-md font-semibold text-gray-800 mb-1">
                      {dept.nameDepartment || "Tên khoa"}
                    </h3>
            ...
            <div className="text-center mb-8">
              ...
              <div className="bg-blue-50 rounded-lg p-3 inline-block shadow">
                <p className="text-gray-700 text-sm">
                  <span className="font-semibold text-blue-600">{selectedBranch?.name}</span>
                  <span className="mx-2 text-gray-400">•</span>
                  <span className="font-semibold text-blue-600">{selectedDepartment?.nameDepartment}</span>
                </p>
              </div>
            </div>
                  ...
                  {specialties.map(spe => (
                    ...
                    >
                      <div className="relative h-40">
                        <img
                          src={spe.imageUrlSpecialty ? `${API_IMG_URL}${spe.imageUrlSpecialty}` : DEFAULT_PLACEHOLDER_IMAGE}
                          alt={spe.nameSpecialty || "Chuyên khoa"}
                          className="w-full h-full object-cover"
                          onError={handleImageError}
                        />
                        <div className="absolute inset-0 bg-gradient-to-t from-black/40 to-transparent" />
                        <h3 className="absolute bottom-3 left-3 right-3 text-lg font-semibold text-white mb-1 line-clamp-2">
                          {spe.nameSpecialty || "Tên chuyên khoa"}
                        </h3>
                      </div>
                      <div className="p-4">
