...
import { Calendar as CalendarIcon, Clock, User, Stethoscope, MapPin, GraduationCap, Phone, Mail, AlertTriangle, CheckCircle } from 'lucide-react'; // Thêm AlertTriangle, CheckCircle
...
import {
    getDoctorById,
    bookAppointment,
    // checkPatientBooking, // Xem xét có cần không nếu checkUserDailyBookingLimit đủ
...
const ShowDetailDoctor = () => {
    ...
    }, [doctorFullInfo, doctorID, currentUser, generateTimeSlots, getDaysInMonth, currentMonthView]); // Bỏ availableSlotsByDate, selectedDate
    ...
    const handleBooking = () => {
        ...
        setError(null); // Xóa lỗi cũ trước khi mở modal
        setShowConfirmModal(true);
    };

    // Hàm này sẽ thực hiện việc đặt lịch sau khi người dùng xác nhận trên Modal
    const confirmBooking = async () => {
        setShowConfirmModal(false); // Đóng modal
        setBookingLoading(true);
        setError(null);
        try {
            const examinationTime = doctorFullInfo.doctorSchedules?.[0]?.examinationTime || 30;
            ...
            if (alreadyBookedToday) {
                ...
                alert(`Bạn đã có một lịch hẹn với bác sĩ ${doctorFullInfo.doctor.name} vào ngày ${new Date(selectedDate + "T00:00:00").toLocaleDateString('vi-VN')}. Mỗi ngày chỉ được đặt 1 lịch hẹn với cùng một bác sĩ.`);
                return; // Nếu cần ngừng tiếp tục xử lý sau alert
            }
            ...
            const bookingData = {
                doctorId: doctorID, nameDr: doctorFullInfo.doctor.name, date: selectedDateISO,
                slot: slotForBooking, patientId: currentUser.id, patientEmail: currentUser.email,
                symptoms: symptoms || ''
            };

            await bookAppointment(bookingData); // Không cần gán result nếu không dùng
            setIsBooked(true);
            ...
            console.error('Booking failed after confirmation:', err);
            ...
            // Không dùng alert ở đây nữa, lỗi sẽ hiển thị trên UI
        } finally {
            ...
            <div className="max-w-4xl mx-auto p-4 sm:p-6">
                <div className="bg-white rounded-lg shadow-xl overflow-hidden">
                    ...
                    <BookingSection
                        ...
                        handleBooking={handleBooking} // Giờ đây chỉ mở modal
                        doctorFullInfo={doctorFullInfo} bookingLoading={bookingLoading} currentUser={currentUser}
                        ...
                        confirmBooking={confirmBooking} // Hàm thực hiện đặt lịch
                        formatCurrency={formatCurrency} // Modal cần formatCurrency
                    />
                </div>
            </div>
        </>
...
const ConfirmBookingModal = ({
    ...
    const confirmInfo = [
        ...
        ...(doctorFullInfo.doctorSchedules?.length > 0 && doctorFullInfo.doctorSchedules[0].consultationFee != null ? [{
            icon: '💰', label: 'Phí khám (dự kiến):',
            value: formatCurrency(doctorFullInfo.doctorSchedules[0].consultationFee),
...
const BookingSection = ({
    ...
    showConfirmModal, setShowConfirmModal, confirmBooking, formatCurrency // Thêm formatCurrency
}) => {
    ...
    return (
        <div className="p-4 sm:p-6">
            ...
            {currentUser ? ( /* ... Thông tin bệnh nhân ... */ <div className="mb-4 sm:mb-6 p-3 sm:p-4 bg-blue-50 rounded-lg border border-blue-200"> <h3 className="text-md sm:text-lg font-medium mb-2 flex items-center text-blue-700"> <User className="w-5 h-5 mr-2" /> Thông tin bệnh nhân </h3> <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-1 text-sm text-gray-700"> <div><span className="font-semibold">Họ tên:</span> {currentUser.name}</div> <div><span className="font-semibold">Email:</span> {currentUser.email}</div> </div> </div>
            ) : ( /* ... Vui lòng đăng nhập ... */ <div className="mb-4 sm:mb-6 p-3 sm:p-4 bg-yellow-50 border border-yellow-300 rounded-lg"> <p className="text-yellow-700 text-sm sm:text-base">Vui lòng đăng nhập để đặt lịch khám.</p> </div>)}
            ...
            <div className="text-center mt-6 sm:mt-8">
                <button
                    onClick={handleBooking} // Sẽ mở modal
            ...
            {showConfirmModal && (
                <ConfirmBookingModal
                    ...
                    bookingLoading={bookingLoading} // Truyền bookingLoading cho modal
                />
            )}
        </div>
    );
...
const BookingSuccess = ({ doctorFullInfo, selectedDate, selectedTime, symptoms, formatCurrency, resetBooking, currentUser }) => {
    ...
    const successInfo = [{ icon: User, label: 'Bệnh nhân:', value: currentUser.name }, { icon: Mail, label: 'Email:', value: currentUser.email }, { icon: User, label: 'Bác sĩ:', value: doctorFullInfo.doctor.name }, { icon: CalendarIcon, label: 'Ngày khám:', value: displayDate }, { icon: Clock, label: 'Thời gian:', value: displaySlot }, ...(doctorFullInfo.doctorSchedules?.length > 0 && doctorFullInfo.doctorSchedules[0].consultationFee != null ? [{ icon: '💰', label: 'Phí khám (dự kiến):', value: formatCurrency(doctorFullInfo.doctorSchedules[0].consultationFee), className: 'text-green-700 font-semibold' }] : []), { icon: Stethoscope, label: 'Triệu chứng:', value: symptoms || 'Không có', start: true }];
